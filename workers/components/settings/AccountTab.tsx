'use client'

interface User {
  name: string
  email: string
  role: string
}

interface AccountTabProps {
  user: User | null
}

export default function AccountTab({ user }: AccountTabProps) {
  return (
    <div className="space-y-6">
      {/* Profile Info */}
      <div className="card">
        <h2 className="font-heading text-xl text-relic-gold mb-4">Profile</h2>
        <div className="space-y-4">
          <div className="flex items-center justify-between py-3 border-b border-baked-clay/20">
            <span className="text-text-slate font-mono text-sm">Name</span>
            <span className="text-sand">{user?.name}</span>
          </div>
          <div className="flex items-center justify-between py-3 border-b border-baked-clay/20">
            <span className="text-text-slate font-mono text-sm">Email</span>
            <span className="text-sand">{user?.email}</span>
          </div>
          <div className="flex items-center justify-between py-3">
            <span className="text-text-slate font-mono text-sm">Role</span>
            <span className={`px-2 py-1 rounded text-xs font-mono ${
              user?.role === 'admin' 
                ? 'bg-relic-gold/20 text-relic-gold' 
                : 'bg-text-slate/20 text-text-slate'
            }`}>
              {user?.role}
            </span>
          </div>
        </div>
      </div>

      {/* Authentication Method */}
      <div className="card">
        <h2 className="font-heading text-xl text-relic-gold mb-4">Authentication</h2>
        <div className="flex items-center gap-4 p-4 bg-obsidian/50 rounded-lg border border-baked-clay/20">
          <div className="w-10 h-10 bg-white rounded-full flex items-center justify-center">
            <svg className="w-6 h-6" viewBox="0 0 24 24">
              <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
              <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
              <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/>
              <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/>
            </svg>
          </div>
          <div className="flex-1">
            <p className="text-sand font-semibold">Google Account</p>
            <p className="text-text-slate text-sm">{user?.email}</p>
          </div>
          <div className="px-3 py-1 bg-status-active/20 text-status-active text-xs font-mono rounded">
            Connected
          </div>
        </div>
        <p className="text-text-slate text-sm mt-4">
          Your account is secured with Google Sign-In. No separate password is required.
        </p>
      </div>

      {/* Security Info */}
      <div className="card">
        <h2 className="font-heading text-xl text-relic-gold mb-4">Security</h2>
        <div className="space-y-3 text-text-slate text-sm">
          <p>• Your session expires after 7 days of inactivity</p>
          <p>• Sign out from all devices by logging out and back in</p>
          <p>• Contact an administrator to update your account details</p>
        </div>
      </div>
    </div>
  )
}
